...
dir_in = (Path(ruta_base)/ 'Backend/Input/').resolve()
...
path_report_format = (Path(dir_in)/'format_reports/formato.xlsx').resolve()
path_last_anexo = (Path(dir_output)/'anexo_final.xlsx').resolve()
dir_output_PDFs = (Path(dir_output)/'report_parts/').resolve()
format_anexo = (Path(dir_in)/ 'format_reports/format_anexo.xlsx').resolve()
directory_risk = (Path(dir_in)/ 'adm_risk').resolve()
directory_ADP = (Path(dir_in)/ 'ADP').resolve()
directory_sigemet = (Path(dir_in)/ 'Sigemet').resolve()
directory_social_programs = (Path(dir_in)/ 'programs_social').resolve()